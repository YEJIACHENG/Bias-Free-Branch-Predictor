    ...
    {	
	  if(pred->class == BPredPerceptron)
	  {
		...
		*************************************************************/
		...
		int needUpdateWeights;
		int needUpdateRS;
		needUpdateWeights = 0;
		needUpdateRS = 0;
		
		// BST = config.BF_neural.BST
		
		int size_BST = sizeof(config.BF_neural.BST);
		...
		if (pred->dirpred.bimod->config.BF_neural.BST[ baddr % size_BST] == 0) {
			// not exists
			
			if (taken) {
				pred->dirpred.bimod->config.BF_neural.BST[ baddr % size_BST] = 3;
			}else {
				pred->dirpred.bimod->config.BF_neural.BST[ baddr % size_BST] = 1;
			}
		} else if ((u->prediction != taken) && ((pred->dirpred.bimod->config.BF_neural.BST[ baddr % size_BST] == 1) || (pred->dirpred.bimod->config.BF_neural.BST[ baddr % size_BST] == 3))) {
			
			// change to non-biased branch
			pred->dirpred.bimod->config.BF_neural.BST[ baddr % size_BST] = 2;
			needUpdateWeights = 1;
		} else if ((pred->dirpred.bimod->config.BF_neural.BST[ baddr % size_BST] == 2) &&  (u->output < THETA(pred->dirpred.bimod->config.BF_neural.prcpt_history)) && (u->output > -THETA(pred->dirpred.bimod->config.BF_neural.prcpt_history) ) {
		...
		if (pred->dirpred.bimod->config.BF_neural.BST[ baddr % size_BST] == 2) {
			needUpdateRS = 1;
		}
