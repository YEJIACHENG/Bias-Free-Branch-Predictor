...
struct bpred_dir_t {
  ...
  union {
	...
	struct {
	  ...
	  int h_t; /* the number of recent branches tracked using the conventional
perceptron predictor component */

	  int RS[4];
	  
	  int GHR[1000];/* global 1000 non-biased history of branches */
	  
	  md_addr_t A[1000]; /* 1000 non-biased addresses */
	 
	  int P;
	  
	  
	  int GHR_unfiltered[2000] /* Global history register containing the outcomes of all branches. */
