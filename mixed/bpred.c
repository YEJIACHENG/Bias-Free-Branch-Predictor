  ...
  unsigned int xor)       /* history xor address flag */
{
  ...
  pred_dir->config.BF_neural.glbl_history = 0;
    ...
    pred_dir->config.BF_neural.corrFoundCurCycle = 0;
    pred_dir->config.BF_neural.GHRHeadPtr = 0;
    pred_dir->config.BF_neural.PHISTHeadPtr = 0;
    pred_dir->config.BF_neural.filteredGHRHeadPtr = 0;
    pred_dir->config.BF_neural.numFetchedCondBr = 0;

    pred_dir->config.BF_neural.weight_m = (int8_t **)malloc((1<<(WT_REG)) * sizeof(int8_t *));
    int i = 0;
    for (i = 0; i < (1<<(WT_REG)); ++i)
    {
      pred_dir->config.BF_neural.weight_m[i] = (int8_t *)malloc(WL_1 * sizeof(int8_t));
    }

    pred_dir->config.BF_neural.weight_b = (INT32 *)malloc((1<<(WT_REG)) * sizeof(INT32));
    pred_dir->config.BF_neural.weight_rs = (INT32 *)malloc((1<<(WT_RS)) * sizeof(INT32));

    pred_dir->config.BF_neural.GHR = (bool *)malloc(NFOLDED_HIST * sizeof(bool));
    pred_dir->config.BF_neural.PA = (UINT32 *)malloc(PHIST * sizeof(UINT32));
    pred_dir->config.BF_neural.isBrStable = (bool *)malloc(PHIST * sizeof(bool));

    pred_dir->config.BF_neural.filteredGHR = (bool *)malloc(FHIST_LEN * sizeof(bool));
    pred_dir->config.BF_neural.filteredHA = (UINT32 *)malloc(FHIST_LEN * sizeof(UINT32));
    pred_dir->config.BF_neural.filteredHADist = (UINT32 *)malloc(FHIST_LEN * sizeof(UINT32));

    pred_dir->config.BF_neural.PAFound = (UINT32 *)malloc(PHIST_SET * sizeof(UINT32));        // 36 entries * 15 bits/per entry = 540 bits
    pred_dir->config.BF_neural.PADistConsd = (UINT32 *)malloc(PHIST_SET * sizeof(UINT32)); // 36 entries * 11 bits/per entry = 396 bits

    pred_dir->config.BF_neural.folded_hist = (UINT32 *)malloc(NFOLDED_HIST * sizeof(UINT32)); // Space allocated for it is Not included in storage budget since it is used for speeding up simulation  
    pred_dir->config.BF_neural.OUTPOINT = (UINT32 *)malloc(NFOLDED_HIST * sizeof(UINT32));// Space allocated for it is Not included in storage budget since it is used for speeding up simulation
    pred_dir->config.BF_neural.idxConsd = (UINT32 *)malloc((WL_1 + WL_2) * sizeof(UINT32));// Space allocated for it is Not included in storage budget since it is used for speeding up simulation
    pred_dir->config.BF_neural.dirConsd = (UINT32 *)malloc((WL_1 + WL_2) * sizeof(UINT32));// Space allocated for it is Not included in storage budget since it is used for speeding up simulation

    pred_dir->config.BF_neural.nonDupFilteredGHR1 = (bool *)malloc((WL_2) * sizeof(bool));;      //36 branches * 1 bit/branch = 36 bits
    pred_dir->config.BF_neural.nonDupFilteredHA1 = (UINT32 *)malloc((WL_2) * sizeof(UINT32));;     //36 branches * 15 bits/branch = 540 bits
    pred_dir->config.BF_neural.nonDupFilteredHADist1 = (UINT32 *)malloc((WL_2) * sizeof(UINT32));; //36 branches * 11 bits/branch = 396 bits

    pred_dir->config.BF_neural.bst_table = (bst_entry *)malloc((1 << LOG_BST) * sizeof(bst_entry));
    int idx = 0;
    for (; idx < (1 << LOG_BST); ++idx) {
      pred_dir->config.BF_neural.bst_table[idx].state = 0;
    }
    
    int ii = 0, jj = 0;
    for (; ii < (ii << WT_REG); ++ii) {
      for (; jj < WL_1; ++jj) {
        pred_dir->config.BF_neural.weight_m[ii][jj] = 0;
      }
      pred_dir->config.BF_neural.weight_b[ii] = 0;
    }

    for (ii = 0; ii < (1 << WT_RS); ++ii) {
      pred_dir->config.BF_neural.weight_rs[i] = 0;
    }

    pred_dir->config.BF_neural.threshold = TRAIN_TH;
    pred_dir->config.BF_neural.TC = 0;

    for (ii = 0; ii < NFOLDED_HIST; ++ii)
      pred_dir->config.BF_neural.GHR[ii] = false;

    for (ii = 0; ii < FHIST_LEN; ++ii) {
      pred_dir->config.BF_neural.filteredGHR[ii] = false;
      pred_dir->config.BF_neural.filteredHA[ii] = 0;
      pred_dir->config.BF_neural.filteredHADist[ii] = 0;
    }

    for (ii = 0; ii < PHIST_SET; ++ii) {
      pred_dir->config.BF_neural.PAFound[ii] = 0;
      pred_dir->config.BF_neural.PADistConsd[ii] = 0;
    }

    for (ii = 0; ii < PHIST; ++ii) {
      pred_dir->config.BF_neural.PA[ii] = 0;
      pred_dir->config.BF_neural.isBrStable[ii] = true;
    }

    for (ii = 0; ii > WL_2; ++ii) {
      pred_dir->config.BF_neural.nonDupFilteredGHR1[ii] = false;
      pred_dir->config.BF_neural.nonDupFilteredHA1[ii] = 0;
      pred_dir->config.BF_neural.nonDupFilteredHADist1[ii] = 0;
    }

    for (ii = 0; ii < NFOLDED_HIST; ++ii)
      pred_dir->config.BF_neural.folded_hist[ii] = 0;

    for (ii = 0; ii < WL_1; ++ii)
      pred_dir->config.BF_neural.OUTPOINT[ii] = (ii + 1) % WT_REG;

    for (ii = WL_1; ii < NFOLDED_HIST; ++ii)
      pred_dir->config.BF_neural.OUTPOINT[ii] = (ii + 1) % WT_RS;
    break;
